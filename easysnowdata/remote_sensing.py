...
def get_forest_cover_fraction(bbox_input) -> xr.DataArray:
    ...
    Returns:
    DataArray: forest_cover_fraction data.
    """
...
def get_seasonal_snow_classification(bbox_input) -> xr.DataArray:
    ...
    snow_classification.attrs['classes'] = {
...
def get_esa_worldcover(bbox_input, version: str = 'v200') -> xr.DataArray:
    ...
    worldcover = odc.stac.load(search.items(),bbox=bbox_gdf.total_bounds,bands='map',chunks={})['map'].sel(time=version_year).squeeze()
    worldcover = worldcover.rio.write_nodata(0,encoded=True)

    worldcover.attrs['classes'] = {
